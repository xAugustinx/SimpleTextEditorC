...
void * pisanieWPliku(void * arg)
{
    ...
    while (turnON)
    {
        ...
        if (czyWpisany && !HOMEMode)
        {
                ...
                for (int i = pointerOfUser - 1; i >= 0 && i < rozmiarTablicy; i--) {
                ...
                for (int i = odKtoregoZaczacWypisywanie+1; i < rozmiarTablicy-1; i++) {
                    if (i > 1&& i < rozmiarTablicy)
                    {
...
int main(int argc, char *qrgv[])
{
    ...
    for (int i = 1; i < rozmiarTablicy && czyUruchomionyPrzyStarcie; i++){
        fseek(plikKodu,i-1,0);
        plikWFormieTablicyChar[i] = fgetc(plikKodu);
    }
